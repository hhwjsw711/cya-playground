...
- tasks: `by_projectId`, `by_assigneeId`, `by_assigneeIds`, `by_projectId_and_status`, `by_projectId_and_assigneeId`, `by_projectId_and_assigneeIds`
- comments: `by_taskId`
...
原字段 `assigneeId`（单选）扩展为 `assigneeIds`（数组），为保证向后兼容：
...
任务类型 `taskType` 为 union literal，新增类型时需同步修改所有文件：
...
- `convex/schema.ts` - v.union literal
- `convex/http.ts` - validTaskTypes 数组（POST + PATCH 两处）
- `convex/analytics.ts` - TASK_TYPE_LABELS
